...
export function AlertRulesPage({ nodes }: AlertRulesPageProps) {
    ...
    <>
      <section className="rounded-2xl border border-border bg-card p-5 shadow-glass">
        <div className="flex flex-col gap-3 border-b border-border pb-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-xl font-black text-foreground">告警规则</h2>
            <p className="mt-1 text-sm font-semibold text-muted-foreground">
              配置基于指标的告警规则，当条件持续满足时发送通知。
            </p>
          </div>
          <button
            type="button"
            onClick={openCreateForm}
            className="min-h-11 cursor-pointer rounded-2xl bg-primary px-4 text-sm font-black text-primary-foreground shadow-sm transition hover:brightness-110 focus:outline-none focus:ring-4 focus:ring-primary/20"
          >
            创建规则
          </button>
        </div>
