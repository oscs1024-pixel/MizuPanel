...
export function SystemSettingsPage({ settings, selectedRetention, saving, message, error, onSelectRetention, onSave }: { settings?: SettingsResponse, selectedRetention: RangeOption, saving: boolean, message?: string, error?: string, onSelectRetention: (retention: RangeOption) => void, onSave: () => void }) {
  return (
    <section aria-label="系统设置" className="overflow-hidden rounded-2xl border border-border bg-card shadow-glass">
      ...
      <div className="grid gap-4 p-4 xl:grid-cols-[0.66fr_0.34fr]">
        <section className="rounded-2xl border border-border bg-card p-4 shadow-sm">
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <span className="w-fit rounded-full border border-border bg-muted px-3 py-1 text-xs font-black text-muted-foreground">当前：{settings ? retentionLabel(settings.metrics_retention) : '加载中'}</span>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {retentionOptions.map((option) => (
              <button
                ...
                className={`min-h-36 rounded-2xl border p-4 text-left transition focus:outline-none focus:ring-4 focus:ring-primary/20 ${selectedRetention === option.value ? 'border-primary/40 bg-primary text-primary-foreground shadow-sm' : 'border-border bg-surface text-muted-foreground hover:bg-card hover:text-foreground'}`}
              >
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm font-bold leading-6 text-warning">
            ...
            <button type="button" onClick={onSave} disabled={saving} className="min-h-11 shrink-0 rounded-xl bg-primary px-4 text-sm font-black text-primary-foreground shadow-sm transition hover:brightness-110 focus:outline-none focus:ring-4 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-60">
              {saving ? '保存中...' : '保存设置'}
            </button>
          </div>
        ...
        <aside className="rounded-2xl border border-border bg-surface p-4 text-sm font-bold leading-6 text-muted-foreground">
          <p className="text-xs font-black tracking-[0.18em] text-muted-foreground">说明</p>
          <ul className="mt-3 space-y-3">
            <li className="rounded-xl border border-border bg-card px-4 py-3 shadow-sm">最大保留时间目前限制为 7 天，避免 SQLite 指标表无限增长。</li>
            <li className="rounded-xl border border-border bg-card px-4 py-3 shadow-sm">历史记录页的可查询范围会跟随这个设置。</li>
            <li className="rounded-xl border border-border bg-card px-4 py-3 shadow-sm">配置文件仍作为首次启动默认值；后台保存后以数据库设置为准。</li>
          </ul>
        </aside>
      </div>
    </section>
